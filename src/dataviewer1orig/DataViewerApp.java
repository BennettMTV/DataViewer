...
public class DataViewerApp implements DrawListener {
	...
	/**
	 * For debugging. Use 'trace' for older debugging messages that you don't want
	 * to see.
	 * 
	...
	/**
	 * Utility function to pull a year integer out of a date string. Supports M/D/Y
	 * and Y-M-D formats only.
	 * 
	 * @param dateString
	 * @return
	 */
	...
	private void updatePlotData() {
		...
		// plot data is a map where the key is the Month, and the value is a sorted map
		// where the key
		// is the year.
		...
		// now run through the raw data and if it is related to the current state and
		// within the current
		...
		for (List<Object> rec : m_dataRaw) {
			...
			if (state.equals(m_selectedState) &&
				...
				if (!m_plotMonthlyMinValue.containsKey(month)
						|| value.compareTo(m_plotMonthlyMinValue.get(month)) < 0) {
					m_plotMonthlyMinValue.put(month, value);
				}
				if (!m_plotMonthlyMaxValue.containsKey(month)
						|| value.compareTo(m_plotMonthlyMaxValue.get(month)) > 0) {
	 ...
	 * @param doGrayscale - if true, return a grayscale value (r, g, b are all
	 *                    equal);
